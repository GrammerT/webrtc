...
bool init_vertbuf2(struct display_capture *dc)
{
...
void load_crop2(struct display_capture *dc, obs_data_t *settings);

static void update_crop_data(struct display_capture *dc, obs_data_t *settings)
{
    int x = obs_data_get_int(settings, "x");
    int y = obs_data_get_int(settings, "y");
    int w = obs_data_get_int(settings, "width");
    int h = obs_data_get_int(settings, "height");
    
    int screen_width = dc->frame.size.width;
    int screen_height = dc->frame.size.height;
    
    obs_data_set_int(settings, "manual.origin.x", x);
    obs_data_set_int(settings, "manual.origin.y", y);
    obs_data_set_int(settings, "manual.size.width", screen_width - x - w);
    obs_data_set_int(settings, "manual.size.height", screen_height - y - h);
}
		...
		obs_source_t *source)
{
    ...
    obs_data_set_int(settings, "crop_mode", CROP_MANUAL);
    update_crop_data(dc, settings);
    
	load_crop2(dc, settings);

	dc->display = obs_data_get_int(settings, "display");
	pthread_mutex_init(&dc->mutex, NULL);

//    dc->selfRect.origin.x = obs_data_get_int(settings, "x");
//    dc->selfRect.origin.y = obs_data_get_int(settings, "y");
//    dc->selfRect.size.width = obs_data_get_int(settings, "w");
//    dc->selfRect.size.height = obs_data_get_int(settings, "h");
...
void load_crop_mode2(enum crop_mode *mode, obs_data_t *settings)
{
...
void load_crop2(struct display_capture *dc, obs_data_t *settings)
{
	load_crop_mode2(&dc->crop, settings);
...
//    
//    dc->selfRect.origin.x = x;
//    dc->selfRect.origin.y = y;
//    dc->selfRect.size.width = width;
//    dc->selfRect.size.height = height;
...
struct obs_source_info area_capture_info = {
	.id             = "area_capture",
